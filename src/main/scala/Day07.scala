...
object Day07 extends App:
  ...
  def ttsort[A](edges: List[(A, A)])(using Ordering[A]): List[A] =

    @tailrec
    def tsort(deps: SortedMap[A, Set[A]], done: List[A] = List.empty): List[A] =
      ...
      if nodeps.isEmpty && hasdeps.isEmpty then done else
        ...
        tsort(next, done :+ found)

    tsort(edges.foldLeft(SortedMap[A, Set[A]]())((ds, e) =>
  ...
  val answer1: String = ttsort(steps).mkString("")
