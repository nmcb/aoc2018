...
object Day24 extends App:
  ...
  def parseGroup(army: Army, boost: Int, line: String): Group =
    val GrouLine   = """(\d+) units each with (\d+) hit points (?:.*)with an attack that does (\d+) (\w+) damage at initiative (\d+)""".r
    ...
    line match
      case GrouLine(units, hitPower, damage, kind, initiative) =>
  ...
  val input: Seq[String] = Source.fromResource(s"input$day.txt").getLines.toSeq
